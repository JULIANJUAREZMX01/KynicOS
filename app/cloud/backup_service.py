...
class BackupService:
    ...
    async def backup_workspace(self) -> bool:
        """Backup workspace to S3 and remove the local temporary archive."""
        ...
        try:
            ...
            self.s3_client.upload_file(str(tmp_path), self.settings.s3_bucket, s3_key)
            ...
            if tmp_path and tmp_path.exists():
                try:
                    ...
                    logger.warning(f"Could not remove temporary backup {tmp_path}: {cleanup_error}")
